...
from django.views.generic import (
    CreateView, UpdateView, FormView, ListView, RedirectView, DetailView
)
...
class UserListView(ListView):
    ...
    def _get_filtered_users(self, user, filter_type):
        ...
        return User.objects.filter(**filter_params).distinct().order_by('pk')
...
class ChangePasswordView(LoginRequiredMixin, PasswordChangeView):
    ...
    success_url = reverse_lazy('users:user-details')

    def get_success_url(self):
        """Динамический URL на профиль пользователя."""
        return reverse(
            'users:user-details', kwargs={'pk': self.request.user.pk}
        )
