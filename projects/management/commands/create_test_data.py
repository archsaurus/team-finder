...
from projects.models import Project, UserProject, ProjectStatus
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.SUCCESS(f'Создано {len(users)} пользователей')
        )
        ...
        self.stdout.write(
            self.style.SUCCESS(f'Создано {len(projects)} проектов')
        )
        ...
        for project in projects:
            ...
            if num_participants > 0:
                ...
                selected_users = random.sample(
                    available_users, num_participants
                )
        ...
        self.stdout.write(
            self.style.SUCCESS(f'Участники распределены')
        )
        ...
        self.stdout.write(
            self.style.SUCCESS(f'Добавлено {favorite_counts} избранных')
        )
        ...
        avg_participants = \
            sum(project_participant_counts) / len(project_participant_counts)
        ...
        self.stdout.write(f'  testuser1@example.com / {PASSWORD}')
